...
adminRouter.use(authenticate, loadCurrentUser, requireAdmin)
adminRouter.get('/metrics', c.metrics)
adminRouter.get('/users', c.users)
adminRouter.patch('/users/:id', c.updateUser)
adminRouter.get('/campaigns', c.campaigns)
adminRouter.patch('/campaigns/:id', c.updateCampaign)
adminRouter.delete('/campaigns/:id', c.deleteCampaign)
